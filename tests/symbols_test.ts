...
//
// The `.vl` corpus (run_test.ts) is black-box behavioral and can't exercise the
// symbol table, so these drive `parseSymbols` directly. Run with:
//   deno test -A --no-check tests/symbols_test.ts
// (the `deno task test` task targets only run_test.ts; this file is a sibling.)

import { parseSymbols, stringifyType } from "../compiler/compile.ts";
import type { Position } from "../compiler/ast.ts";

// Tiny structural-equality assert (the repo has no std import map; run_test.ts
