// The `-O` / `-O3` release-shape suite's MACHINERY — the fixture tables, the
// `wasm-dis` counters and the build helper. The tests that read it are the four
// `selfhost_native_release*_test.ts` files, one per table, because `deno test
// --parallel` gives one worker per FILE and all 36 of them in one file ran 33 s
// serially (docs/internals/perf-opportunities-2026-09.md §A3). Nothing is
// registered here; every `Deno.test` lives in one of those files, unchanged.
...
export const LOOPS = `${ROOT}/tests/fixtures/opt-loop`;
export const BENCH = `${ROOT}/bench`;
export const CASES = new URL("../cases/", import.meta.url);

const GATED = Deno.env.get("SELFHOST_NATIVE_ALIGN") === "1";
const haveBin = exists(VL);
const haveSeed = exists(COMPILER);
const haveOpt = exists(WASM_OPT) && exists(WASM_DIS);
export const ENABLED = GATED && haveBin && haveSeed && haveOpt;
if (GATED && !ENABLED) {
  const why = !haveBin
    ? "missing vl binary"
    : !haveSeed
...
export const MELT_TABLE: Array<{ fixture: string; none: number; O: number; O3: number }> = [
...
export const BYTES_TOL_FRAC = 0.03;
export const BYTES_TOL_MIN = 16;
...
export const SHAPE_TABLE: Array<{ bench: string; axis: string; O: ShapePins; O3: ShapePins }> = [
  ...
  // unroll factor moves.
  {
    ...
    //
    // 2026-09-23, immutable module consts: `const ALPHA` is now an immutable global, so `-O3`
...
export const shapeOf = (wat: string, bytes: number, want: ShapePins): ShapePins => {
  const got: ShapePins = {
    bytes,
    fns: funcCount(wat),
...
export const exact = ({ bytes: _bytes, ...rest }: ShapePins) => rest;
...
export const logsOf = (s: string) =>
  [...s.matchAll(/^\s*\/\/\s*@log (.*)$/gm)].map((m) => m[1]);

export const vl = async (args: string[], env: Record<string, string> = {}) => {
...
export const allocSites = (wat: string) =>
...
// scan.
export const loopShapes = (wat: string): [number, number, number] => {
...
export const countOf = (wat: string, re: RegExp) => (wat.match(re) ?? []).length;
...
export const funcCount = (wat: string) => countOf(wat, /^ \(func /gm);
...
export const buildAndCount = async (src: string, flags: string[], tmp: string) => {
...
export const rustList = (src: string, name: string): string[] => {
