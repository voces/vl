...
// ---- focused round-trip / comment-survival snippets ----------------------

// Representative snippets covering the v1 scope: declarations, functions,
// control flow, expressions, literals, and comments. Each asserts the three
// guarantees directly (not only over the corpus): idempotence, AST-equivalence
// (`parse(format(s))` projects to the same tree as `parse(s)`), and that no
// comment line is lost.
const ROUND_TRIP_SNIPPETS: string[] = [
  "let x = 1\nconst y: i32 = 2\nx = x + y\n",
  "let x = 1\nx += 1\nx -= 2\nx *= 3\n", // compound assignment survives verbatim
  "function add(a: i32, b: i32): i32 {\n  return a + b\n}\nprint(add(1, 2))\n",
  "function one(x: i32) x * 2\nprint(one(3))\n", // one-line function body
  "let a = 5\nif a > 0 {\n  print(1)\n} else if a < 0 {\n  print(2)\n} else {\n  print(3)\n}\n",
  "for i in 0 to 3 {\n  print(i)\n}\n",
  "let n = 0\nwhile n < 10 {\n  n = n + 1\n}\nprint(n)\n",
  "let o = { x: 1, y: 2 }\nlet a = [1, 2, 3]\nprint(o.x + a[0])\n",
  'let s = "hi"\nlet u: boolean | i32 = 5\nprint(s.length)\n',
  "outer: for a in 1 to 5 {\n  if a == 3 { break outer }\n}\n",
  "let a: i32 | null = null\nlet b: i32 | null = 2\nprint(a ?? b ?? 100)\n",
  "function f(x: string | i32): i32 {\n  if x is i32 { return x }\n  return 0\n}\nprint(f(3))\n",
];

for (const snippet of ROUND_TRIP_SNIPPETS) {
  Deno.test(`fmt round-trip: ${JSON.stringify(snippet.slice(0, 40))}…`, () => {
    const once = format(snippet);
    // Idempotent.
    assertEquals(format(once), once, "not idempotent");
    // AST-equivalent (semantics preserved through formatting).
    const before = projectProgram(snippet);
    const after = projectProgram(once);
    assertEquals(before.ok, true, "snippet did not parse cleanly");
    assertEquals(after.ok, true, "formatted snippet did not parse cleanly");
    assertEquals(after.tree, before.tree, "formatting changed the AST");
  });
}

// A `//` comment ABOVE a statement and a `//` comment TRAILING a statement both
// survive formatting (the headline comment-preservation guarantee).
Deno.test("fmt: leading and trailing comments survive", () => {
  const src = "// leading note\nlet x = 1 // trailing note\n";
  const out = format(src);
  assertEquals(out.includes("// leading note"), true, "lost leading comment");
  assertEquals(out.includes("// trailing note"), true, "lost trailing comment");
  // The leading comment stays on its own line; the trailing stays on the `let`.
  assertEquals(out, "// leading note\nlet x = 1 // trailing note\n");
});

// A doc comment (`///`) is likewise retained verbatim.
Deno.test("fmt: doc comments survive", () => {
  const src = "/// doc for f\nfunction f() {\n  return 1\n}\n";
  assertEquals(format(src).includes("/// doc for f"), true);
});
