#!/usr/bin/env bash
# Build the native `vl` RELEASE binary — a single self-contained file carrying BOTH
# the compiler seed and std (`cargo build --features embed-seed`; see scripts/vl-host).
# std rides in `src/std_embedded.rs`, generated by `deno task gen-std`; the feature
# bakes the seed AND makes this a distribution build, which resolves neither from the
# filesystem (D1573/D1574). A pin is this one file: nothing is copied beside it.
