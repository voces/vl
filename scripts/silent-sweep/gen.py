#!/usr/bin/env python3
"""
Representation x position x construct x runtime-input x SCOPE sweep generator for VL.
...
Every cell:
  ...
  * is generated at BOTH scopes (see below), so every module-scope cell has a
    function-scope twin differing in exactly one coordinate.

THE SCOPE AXIS — the EXECUTING BODY's storage class, not the binding's.
  `scope=fn`   the reading statements sit inside `function reader() { ... }`, which the
               module then calls.  This is what every earlier run of this grid measured.
  `scope=mod`  the SAME statements are emitted at module top level, with no enclosing
               function and no call.  Module-scope bindings are GLOBAL cells and resolve
               their rep through a different classifier ladder
               (`letNulScalarListKind` -> `exprNulScalarListKind`) than function locals
               (`letCellKind` / `letInitCellKind`), so a rep that lowers inside a function
               is not thereby known to lower outside one.

  The `global` POSITION already varied where the BINDING sits; it never varied where the
  READ executes, so `global` at `scope=fn` (bind outside, read inside) and `global` at
  `scope=mod` (both outside) are different cells and both are generated.

  `pos=param` is the ONE structurally unrepresentable combination at module scope — a
  parameter requires a function to be a parameter of — and is skipped deliberately.  The
  skip count is printed and lands in the manifest, because a silently truncated axis reads
  as "covered everything" when it did not.  `pos=capture` at module scope is generated and
  is a real program, but note that its captured binding is then a global, so it covers the
  same storage class as `global`.

Usage: gen.py <outdir> [--scopes fn,mod]
"""
...
POSITIONS = [
    ...
    "mapget",
    # BOTH: a declared-nullable value type read at a key that may be missing
    "mapval_miss",
]

def build(rep_name, nul, pos, con_name, read_name, inp, outdir, idx, spell="inline",
          scope="fn"):
    # A parameter needs a function to be a parameter of: `param` is the one position with
    # no module-scope spelling at all.  Skipped here so the count is recorded rather than
    # the axis silently truncated.
    if scope == "mod" and pos == "param":
        return None
    R = REPS[rep_name]
    ...
    if pos == "const_local":
        ...
        body.append('  mm["kk"] = src()')
        access = 'mm["kk"]'
    elif pos == "mapval_miss":
        # The map's value type is DECLARED nullable AND the key may be MISSING.
        # `mapval` only ever reads a key it just stored, so the miss arm of a
        # declared-nullable value type was never generated by this grid at all — which
        # is exactly D19's coordinate.  One thing separates this from `mapget`: there
        # the `| null` comes from the index read alone, here the value type carries it
        # too, so a miss and a stored null must arrive at the same arm.
    ...
    if pos == "mapget":
        ...
        # The value type stays DECLARED nullable; only the STORED value is present,
        # because the null under test comes from the missing key, not from the store.
        src_ret = R["vals"][0][0]
    ...
    lines += pre
    if scope == "mod":
        # MODULE-SCOPE execution context: the statements that would sit inside
        # `function reader()` are emitted at module top level instead, with no enclosing
        # function and no call.  Stripping exactly two leading spaces preserves the
        # relative indentation of any nested block the construct built.
        lines += [(l[2:] if l.startswith("  ") else l) for l in body]
    elif pos == "param":
...
def main():
    ...
    if "--scopes" in sys.argv:
        scopes = sys.argv[sys.argv.index("--scopes") + 1].split(",")
    ...
    def emit(leg, rep_name, nul, pos, con, read_name, inp, spell="inline"):
        nonlocal n
        for scope in scopes:
            got = build(rep_name, nul, pos, con, read_name, inp, outdir, n, spell, scope)
            if got is None:
                skipped.append([leg, rep_name, int(nul), pos, con, read_name, inp,
                                spell, scope])
                continue
            text, expected = got
            name = f"c{n:05d}"
            ...
            cells[name] = dict(leg=leg, rep=rep_name, nul=int(nul), pos=pos, con=con,
                               read=read_name, inp=inp, spell=spell, scope=scope,
                               expected=expected)
            n += 1
    ...
    for rep_name, R in REPS.items():
        primary = R["reads"][0][0]
        for pos in PLACE_POSITIONS + ["mapget", "mapval_miss"]:
    ...
    with open(os.path.join(outdir, "manifest.json"), "w") as fh:
        json.dump(dict(cells=cells, skipped=skipped, scopes=scopes), fh)
    print(f"generated {n} cells, {len(skipped)} skipped combinations")
    # A skipped combination is a HOLE in the cross product; print it broken down so a
    # truncated axis cannot read as full coverage.
    byscope = collections.Counter(s[8] for s in skipped)
    bycell = collections.Counter(c["scope"] for c in cells.values())
    for sc in scopes:
        print(f"  scope={sc:4s} cells {bycell[sc]:6d}   skipped {byscope[sc]:5d}")
    reasons = collections.Counter((s[8], s[3]) for s in skipped)
    print("  skips by (scope, position), most common first:")
    for (sc, pos), k in reasons.most_common(8):
        print(f"    {sc:4s} {pos:14s} {k:5d}")
