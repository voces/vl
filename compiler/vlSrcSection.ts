// THE `vl-src` CUSTOM SECTION, READ IN TYPESCRIPT — the JS/TS host's half of ROADMAP row 22.
//
...
// trapping statement.
//
// THE JOIN KEY IS THE SAME NUMBER IN EVERY ENGINE, and that is measured rather than assumed.
...
// the native one join identically and one expected block grades both — and V8 is V8 whether
// it is Deno's or the browser's, so `sourceFrames` resolves a playground trap unchanged.
//
// This file is dependency-free and host-agnostic on purpose: it takes bytes and a stack
// string and returns lines. It lives in `compiler/` beside `coreTypes.ts` so BOTH the test
// hosts (`tests/support/runWasm.ts`) and the browser playground (`playground/src/*`) import
// the one reader; a stale re-export shim sits at `tests/support/vlSrcSection.ts`.
