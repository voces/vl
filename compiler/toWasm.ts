  ...
  const toExpressionRaw = (node: VLProgramNode | VLStatement): number => {
    ...
    switch (node.type) {
      ...
      case "Call": {
        ...
        // properties have no closure to dispatch through — lower them directly to
        // their wasm helper (or an inline `array.get`). The receiver and args are
        // plain i32-arrays / i32s.
        if (node.callee.type === "PropertyAccess") {
          ...
          if (rt.type === "Object" && rt.name === "string") {
            const at = arrayType(i32Type);
            const prop = node.callee.property;
            const argExpr = (k: number) =>
              withDesiredType(
                k === 0 && (prop === "indexOf" || prop === "includes")
                  ? rt
                  : i32Type,
            ...
            if (prop === "charCodeAt") {
              // `array.get` — bounds are the caller's responsibility, matching
              // `s[i]` (no JS NaN result; an out-of-range index traps).
              return m.array.get(
                toExpression(recv),
                argExpr(0),
                binaryen.i32,
