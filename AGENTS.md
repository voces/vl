...
- **Run / build / check / fmt a file:** the native `vl` binary (`scripts/vl-host`, built with
  ...
  (`wasm-opt`/`wasm-dis`; `brew install binaryen`), soft no-op when absent. `vl check [path]` reports
  errors + lint (warnings/hints) over a file or directory (default cwd; `--exclude`/`--ignore <glob>`),
  `--severity <level>` (gate + display floor), `--concise`, `--codegen`.
  Not yet ported to `vl` (`check --fix`) still run via
