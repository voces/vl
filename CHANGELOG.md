...
- **B6 Growable `T[]` (core)** — `{backing,len,cap}` WasmGC struct monomorphized per element type; 2× growth; `[...]` seed; `a[i]`/`a[i]=v` trap on OOB; `.length`/`.capacity`/`.get(i)`/`push`/`pop`/`clear`/`+`; for-in + structural equality updated. (`compiler/builtins/lists.ts`; → `docs/collections-design.md`)
