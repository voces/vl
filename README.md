...
| `vl build <file.vl>` | Compile to WebAssembly (`-o <out.wasm>`; `-O` optimize, `-O3` the release profile, `--wat` also dump text, `--no-validate` skip the instantiability check; `--import-memory` / `--heap-base=` / `--heap-limit=` lay a unit out to share a host-owned memory). |
