...
| `vl check [path]` | Type-check + lint (errors + warnings/hints) a file or directory (default: cwd; `--exclude`/`--ignore <glob>` to skip). Pretty output (carets, TTY color) or `--concise`. `--severity <hint\|info\|warning\|error>` gates the exit code + display floor; `--codegen` also runs the emitter. |
