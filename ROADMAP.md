...
| `plumb:` `std:fs` writes only whole files, from a `u8[]` (PL-020, PL-009) — **RULED (owner, 2026-09-23), awaiting BUILD** | Nothing left to rule; the build owes the std review of the view helper's name. Ruled: `writeFile` widens to `data: u8[] \| Buf`, and `writeFileRange(path, offset, data)` and `appendFile(path, data)` take the same union. There are no `…From` variants. A partial `Buf` write goes through a new copy-free, bounds-checked window helper in `std:buffer`, whose name is open (`window` / `u8view` / `view`, not `slice`). Writes are contiguous for now: an offset past the end is `EINVAL`, and the rule may relax later. There is no truncate primitive (`writeFile(path, [])` starts fresh) and no file handle until scope-exit cleanup exists (row below). Surface, build items 1–7 and self-review: `docs/internals/fs-streaming-design.md` §5–§7. |
