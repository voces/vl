...
| 22 | **B-debug — a trap names a FUNCTION but not a LINE; the index is gone (2026-09-07)** | `print(a[7])` → `vl!__start__` (was `vl!<wasm function 4>`); no frame is an index now, but no frame carries a file or line either, and the message still has no index or length | the emitter has no source path and no offset→line map, so `name@file:line` in the name section — or a custom section the host reads — needs that plumbing first | days |
