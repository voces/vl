...
| 24 | **dogfood `match` over the compiler's own kind ladders** | forty-four conversions over four tranches plus the census fix that reads the scrutinee's declared type; batch 1 of the litunion-parameter sweep lands five more (`eqCoreKindOfTyToken`, `pendingKindOfMf`, `spreadWrapIdx`, `spreadBackIdx`, `binPrec` — 74 members, the frequent-non-operator arm kept first so byte identity holds), `ladder-budget.py` now **389 + 8** | remaining: the other matchable litunion sites by declared type (field reads want the drift-gated table, its own +2-hit landing), the `Node` walkers (excluded), and the two-interleaved-subject sites left as `if` chains. `typecheck.vl` (151) still mostly unsurveyed | days |
...
| 32 | **the playground still lacks two behaviours the "completion" and "code actions" LABELS hide** | measured 2026-09-08 by grepping both hosts for the composing symbol: `stdAutoImportCompletions` (server 4 uses, adapter 0) and `ufcsImportFixes` (server 2, adapter 0); `organizeImportEdits` (server 1, adapter 0) sits under "code actions", which this file grades as IN parity — measured while closing row 10. The UFCS half of completion landed 2026-09-08 and is now a `subBehaviours` row rather than a pin | `playground/src/lspAdapter.ts`: the auto-import completions need `ufcsProbeModules`' browser twin, the import quick-fix needs the diagnostic's `data.modules`; add a `subBehaviours` entry with each so the label cannot hide it again | hours each |
