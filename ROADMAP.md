...
| 24 | **dogfood `match` over the compiler's own kind ladders** | forty-four conversions over four tranches plus the census fix that reads the scrutinee's declared type, then two litunion batches by DECLARED type: batch 1 five parameter sites (`eqCoreKindOfTyToken`, `pendingKindOfMf`, `spreadWrapIdx`, `spreadBackIdx`, `binPrec` — 74 members, the frequent-non-operator arm kept first so byte identity holds), batch 2 the `PrimName` field reads (`t.primName`) and two small-set locals (`tyPrimLeafListKind` over `PrimName \| null`, `eqCmpKindOfNulInner` over `EqCmpKind`) — ten more, `ladder-budget.py` **379 + 8**; then the `PrimName`-field census-attribution table reads a field scrutinee's declaring type from `type X = { f: T }` (drift-gated by `check_fields`) and reveals the one `PrimName`-field dispatch the sweep could not byte-convert (`repTyScalarMask`'s three independent mask-OR `if`s), **380 + 8** — every output population byte-identical | remaining: the large-set locals/fields (`VKind`-31, `TokKind`-74 tested at 2–4 members — a `_`-less match there is a 70-member noise arm, deferred), the two-interleaved-subject sites (`if` chains), the `Node` walkers (excluded). `typecheck.vl` mostly unsurveyed | days |
