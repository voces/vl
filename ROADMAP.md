...
| 24 | **dogfood `match` over the compiler's own kind ladders** | STARTED: `emit_collect.vl`'s six `Ty` dispatches are `_`-less exhaustive `match`es, 38 → 32 on that file, both output populations byte-identical. Seven non-`Node` sites remain there and none is convertible — a `string` scrutinee (4), a predicate rung between two kind rungs (1), a NAME-predicate chain (1), two interleaved subjects (1) | the next file is `emit_classify.vl`'s field-code ladders; measured by `ladder-budget.py` (427 + 8 today) and gated on a `_`-less form — `kind-ladder-lint.md` §The first conversion | days |
