...
| 32 | **the playground adapter composes none of THREE behaviours the "completion" and "code actions" LABELS hide** | measured 2026-09-08 by grepping both hosts for the composing symbol: `stdAutoImportCompletions` (server 4 uses, adapter 0) under "completion", and `ufcsImportFixes` (server 2, adapter 0) and `organizeImportEdits` (server 1, adapter 0) under "code actions" — both rows this file grades as IN parity, so a `knownGap` keyed on `mainMarker` is structurally blind to them. The UFCS-completion residue and the three doc surfaces all landed 2026-09-08; the residue this row named is now DELIVERED as a standing tripwire, not a prose TODO — `SUB_GAPS` in `playground_lsp_parity_test.ts` (#2961) names all three and asserts each STILL HOLDS, so closing one without promoting its row reds the suite | `playground/src/lspAdapter.ts`: the auto-import completions need `ufcsProbeModules`' browser twin, the import quick-fix needs the diagnostic's `data.modules`, and organize-imports needs a `source.organizeImports` action path the playground has none of | hours each |
