...
| 24 | **dogfood `match` over the compiler's own kind ladders** | `emit_collect.vl` 38 → 32 (six `Ty`), `emit_classify.vl` 134 → 105 (twenty-six `Ty` + three `VKind`), every output population byte-identical at every batch. **A LITUNION SET IS MATCHABLE**, nullable included since [D1898](internals/inventory/D1898.md) closed — so `VKind`/`RtKind`/`EqCmpKind` are in scope and the honesty test is whether the fall-through is a real answer, not how many members go unnamed | remaining in `emit_classify.vl`: 78 `Node` (excluded by the rule), 24 lower-volume `VKind`/`MfKind`/`PrimName`/`RtKind`/`EqCmpKind` sites, 1 with a `VKind \| null` scrutinee now UNBLOCKED by D1898 but needing its own landing (a seed carrying the checker change must be on master first), 1 with two interleaved subjects, 1 `Ty` with a predicate rung. `typecheck.vl` (152) is unsurveyed. `ladder-budget.py` reads 398 + 8 today | days |
...
| 29 | **F-day-one — two grammar axes still absent** | `day-one-sampler.md`: operator overloading, mixed-width arithmetic (`match` landed as a `narrowing` spelling, D1885/D1886; multi-param generics as a `pinning` face, D1887/D1888; recursive types as four records with a non-recursive control, D1889) | `scripts/day-one/grammar.py` | half-day per axis |
...
| 32 | **the completion surfaces `///` docs do NOT reach: a user `type` NAME and a struct FIELD** | measured 2026-09-07: `type Pt` documented and in scope is offered as no completion item at all (`symScopeAt` records variable/parameter/function bindings only), and `p.` over a `Pt` whose `x` carries a `///` returns `x` with the bare fence. The UFCS third is BUILT (2026-09-08) — and measuring it found a second gap: the playground adapter has no UFCS path at all, so it offers no such method to document | the type half is a NEW completion source, not a doc gap; the field half needs `memc*` to bank a declaration token the way `scopeResTok` and `ufcTok` do; the playground half is `ufcsCandidatesAt` in `playground/src/lspAdapter.ts`, whose doc then rides `UfcsCandidate.doc` with no new query (pinned in `tests/lsp_ufcs_docs_test.ts`) | hours each |
