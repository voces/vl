...
| --- | --- | --- | --- | --- |
| 1 | **B15 — a nested capturing function cannot be taken as a VALUE.** **NARROWED to D1816 (2026-09-08, witnesses re-run): every earlier hop is built** — the DIRECT call ([D1780](internals/inventory/D1780.md)/[D1781](internals/inventory/D1781.md)), the BINDING hop ([D1782](internals/inventory/D1782.md)), the ARRAY element at ONE pin ([D1794](internals/inventory/D1794.md)), the ARGUMENT hop ([D1795](internals/inventory/D1795.md)), and `return k` (a closure that escapes now RUNS); what stands is the closure delivered as an ARRAY ELEMENT at TWO DIFFERENT pins | `function o(n) { function k(x: i32) { return n }; const fs = [k]; return fs[0](1) }; print(o(2)); print(o(2.5))` is `vl check` rc 0 then `emitProgram: callee is not a function name` — the array literal is ONE arena node shared by the `(i32)=>i32` and `(i32)=>f64` instances, so the keying gives one signature row where two are needed ([D1816](internals/inventory/D1816.md)) | `wasmEmit.vl` `cloArrSlotSigKey` / `rlElemName[slot]` — REP-LAYER work, `scripts/rep-fuzz-check.sh` mandatory | half-day-to-days |
...
| 24 | **dogfood `match` over the compiler's own kind ladders — CLOSED modulo named residue** | forty-four conversions over four tranches, the census fix reading the scrutinee's declared type, two litunion batches (batch 1 five parameter sites incl. `binPrec`'s 74; batch 2 the `PrimName` field reads + two small-set locals), the `PrimName`-field census-attribution table (drift-gated, revealing `repTyScalarMask`), and a final batch converting the last cleanly-convertible single-subject litunion dispatches — `pushKindBit` (`PushKind`), `repTreeVKind` and `rtListVKind` (`RtKind`), byte-identical; `ladder-budget.py` **377 + 8** | the 377 standing silent ladders are the named residue, none a clean byte-identical conversion — split in `docs/internals/kind-ladder-lint.md` §residue: **arena walkers** (`Node` 213 + `Ty` 110, partial `is`-dispatches with real named defaults, a `_`-less match a large noise arm), **deferred-large** (`VKind` 25 + `TokKind` 5, a handful of a 31/74-member litunion), **not-a-dispatch** (the `string`-typed scrutinees, which need their producer given a litunion return first, and the `&&`-guarded compound-condition arms), **multi-subject** (interleaved scrutinees — `numWidensName`, `collectMapFilterUse`, …), and the `RtKind` **`unsup`-shares-default** pair (`repTreeListElemName`, `rtNulVKind` — an `unsup` rung falling to the shared default a `_`-less match cannot replicate) | done |
...
| 27 | **E3 — user wasm runs on the playground's MAIN thread** | `playground/src/runtime.ts:21` instantiates there; `main.ts:36` says so. RESIDUE CLOSED 2026-09-08: the main-thread runtime now names the trapping line — a trap resolves to its `at <line>:<col>  in `fn`` source frames (row 22), the same block `vl run` prints — so what remains is the off-main-thread MOVE itself | `playground/src/{runtime,playground,main}.ts` (a Worker so a long-running or trapping program cannot freeze the editor) | days |
...
| 32 | **SIMD (`v128`) — a first-class vector surface over `Buffer`** (veldt ask #1, "the one gap with no workaround"; S0–S3 SHIPPED — `std:simd` `F32x4` surface, now with lane read-back: `get x/y/z/w` getters and `lane`/`withLane` over a literal-union `Lane` index, one lane instruction at `-O` (D1980 closed); S4 integer/byte shapes next) | veldt's CPU-side rigid-body solver and voxel passes are **~4x off** without it; there is no SIMD spelling in VL today. WASM SIMD is a fixed 128-bit `v128`, linear-memory-only — a GC `(array i8)` cannot be read as a vector, a `Buffer` can — so this is a `Buffer`-tier feature | `docs/internals/simd-design.md` recommends a fixed-width nominal `std:simd` type family (`F32x4`/`I32x4`/`U8x16`/…, distinct `Mask`) over a thin new `__…_v128__` intrinsic family — the `std:buffer` shape exactly. Region: `typecheck.vl` (intrinsics + `v128` prim), `emit_rep.vl` (`REP_SCAL_V128`), `emit_bytes.vl`/`wasmEmit.vl` (the `0xFD` LEB writer + opcode tables), `scripts/vl-host/src` (`--enable-simd`), `std/simd.vl`. **Blocked on ten rulings** the doc lists (O1 library-vs-builtin, O3 compile-time lane index, O4 operator overloading, O6 relaxed-SIMD gate, O7 `std:vec` layer, …) — first buildable slice is S0–S3 (`F32x4` load/store/arith/compare); **row 33 is PREWORK this row depends on for O4** | days (per slice) |
| 33 | **Type-bound UFCS resolution — call a nominal type's methods/operators without importing each** (SIMD prework, DESIGNED) | `std:buffer` already has 44 exports over one type family and every one needs its own import (`std/buffer.vl`); SIMD's recommended surface is ~60 width-suffixed functions (`addF32x4`, not `add`) for exactly this reason. Verified: a constructor import already infers the type with no type import needed (`Buffer(16)` then `.length`, no `Buf` import) but a UFCS method on that value still needs its OWN import even though the checker already knows which module it lives in (D1230); worse, operators ALREADY dispatch with no import at all, through a single whole-program name slot that admits only one declaration ever — two modules each declaring their own `"+"` collide (`redeclared +` / silently drops one), which is a hard wall against SIMD's per-type `"+"`/`"add"` plan, not an ergonomics gap | `docs/internals/type-bound-ufcs-design.md` proposes: on a resolution miss, ask the receiver's inferred type's OWN declaring module (recoverable from its already-mangled `$mN` name, no new arena field) before refusing — explicit lexical resolution still wins unconditionally, so an existing program's meaning cannot change (§F); only `N`'s own module ever contributes (§D orphan rule, no ambiguity). The operator half additionally needs B14's `"[]"` receiver-keyed exception generalized to operators broadly (§G) — SIMD's S3 slice depends on this ruling landing | half-day (methods) + half-day (operators), design owes 7 open questions (§K) |
| 34 | **`std:math` — deterministic transcendentals** (veldt/sunsuz/glean ask + an internal numerics review; DESIGNED) | `sin`/`cos`/`hypot`/`atan2`/`exp`/`pow`/`PI` are all `undeclared identifier` today — confirmed live, `dist/vl` 2026-09-07. `numeric-intrinsics.md` §"What is deliberately absent" already anticipated this module ("a future `std:math` for other users is a separate question and nothing here forecloses it"); three consumers now name a concrete need: veldt (bearing/length math for its rigid-body solver), sunsuz (`atan2`-shaped `turnOf`, with a live cross-host parity harness — `~/sunsuz/tools/headless/coast.mjs`, 2003 bearings × 9 islands, 1e-4 absolute at f32), glean (deterministic trig for exact replay reconstruction) | `docs/internals/std-math-design.md` (contract: deterministic polynomial approximations only, never a host `Math`/libm call, published per-op ABSOLUTE-or-relative error bound, f32 AND f64 first-class surfaces) sits on `docs/internals/numeric-determinism-rulings.md` (the language-level ground: NaN-bit-pattern determinism is a measured engineering commitment across VL's two target engines, not a spec guarantee — relaxed SIMD is the one opt-in exception). Slice 1 = `hypot` (naive, documented) + `atan2` (~1e-7 f64 / ~1e-6 f32 absolute target) + `PI`, gradable against sunsuz's existing harness with no new test infrastructure; slice 2 = `sin`/`cos`/`exp`/`pow`, same contract, harder domains. Owes `std-api-reviewer` at build time (design adds zero exports) and the 5 open questions in `std-math-design.md` §H (naming convention, `pow`'s `ln` dependency, `sin`/`cos`'s v1 domain, `TAU`, `hypot`'s overflow threshold in its doc comment) | half-day (slice 1) + days (slice 2) |
| 35 | **Memory-safety — `Buf` is a forgeable, aliasable structural record** (eight-lens design review, Rust/memory-safety lens) | `import { Buf, storeI32, loadI32 } from "std:buffer"; const forged: Buf = { base: 0, length: 65536 }; storeI32(forged, 100, 0xDEADBEEF as i32); print(loadI32(forged, 100))` — `vl check` rc 0 (one redundant-annotation hint), `vl run` writes and reads arbitrary linear memory, confirmed live 2026-09-07. `std/buffer.vl:13` declares `Buf` as a plain structural record; VL's structural typing means any same-shaped `{base: i32, length: i32}` type, anywhere, silently aliases it | `std/buffer.vl` — `buffer-design.md` §L2b already proved the fix on `Buf`'s own sibling types (`F32View`/`I32View` moved to a `new` newtype: byte-identical on all 8 view fixtures, 12 bytes smaller each) and says in its own words that the identical fix on `Buf` itself "is now a one-word edit whenever it is wanted." One-line source change plus a corpus regrade | hours |
| 36 | **Reconsider O5 — `memory.grow` detaches every host view with no detection signal** (eight-lens design review, Rust/memory-safety lens; revisits an existing ruling) | `buffer-design.md` §B5 measures a host's cached `Float32Array`/`DataView` going silently `byteLength === 0` after any guest call that triggers a grow, with no throw. O5 already RULED "(i) lazy growth, NO epoch export," citing that Emscripten/wasm-bindgen/Go all rely on the same `byteLength === 0` convention with no exported counter — this is a considered ruling, not an oversight | The review's sharper framing, not previously weighed: a Rust *host* gets this hazard caught at compile time for free (`Memory::data` borrows the `Store`); every VL-target host is JS, where the existing convention is easy to get right once and silently regress when an unrelated path starts allocating months later. A monotonic `i32` global plus one export costs little and does not remove the existing `byteLength` contract for hosts that prefer it. Needs an owner ruling to reopen O5, not a build | ruling |
| 37 | **`docs/guide/soundness.md`'s absolute contract wording, plus one now-stale worked example** (eight-lens design review; convergence — Lua, compiler, functional, Rust lenses all independently reached the checker/emitter soundness gap) | The guide's own "Known unsound corners" section names container-element variance as "silently accepted and emits invalid wasm" — verified live 2026-09-07 that this is now FALSE: `vl check` on a `Cat[]`-into-`Animal[]` program refuses LOUDLY (`type-valid (structural width subtyping) but not yet supported by codegen`), already tracked as this table's own row 20 (A9). The doc's headline ("Every well-typed VL program is type-safe at runtime") states the clause-1 (runtime type safety) guarantee with no pointer to the separate, large, and already-tracked clause-2 population (CLAUDE.md: ~226–390 reachable emit-refusal sites by two independent witness samples) a reader could otherwise assume the same absolute sentence covers | `docs/guide/soundness.md` — correct the stale example, add one paragraph distinguishing the clause-1 guarantee the doc proves from the clause-2 population it doesn't mention, with a pointer to where that population is tracked (CLAUDE.md, `scripts/capability-probes/`). Docs only | hours |
| 38 | **No "did you mean" for an undeclared identifier or an import-not-exported name** (eight-lens design review, Swift/ergonomics lens) | `import { Buffer, storeU8 } from "std:buffer"` → `"storeU8" is not exported by "std:buffer"`, no suggestion, though `store8` is one edit away; `print(fF(3))` with `f` declared → `undeclared identifier 'fF'`, no suggestion — both confirmed live 2026-09-07. VL already ships this exact mechanism at two sibling sites, both landed from external-consumer reports: `tyDidYouMean` for unknown type names (D1590) and `methodDidYouMean` for a missed method call on a string/array/map receiver (D1599); neither reaches the import-specifier message (`compiler/driver.vl:3574`, one call site) or the plain undeclared-identifier message (`compiler/typecheck.vl:26842`). Narrower than, and independent of, the UFCS-member "did you mean" `type-bound-ufcs-design.md` §J already scopes as a follow-up to row 33 (that one needs a receiver TYPE to search; a bare name or an import specifier has none) | `compiler/driver.vl` (import-not-exported), `compiler/typecheck.vl` (undeclared identifier) — D1590/D1599 are the direct precedent for the fix shape: an edit distance (`tyEditDist`) over the plausible candidate set, suffixed onto the existing message | hours-to-half-day |
...
| A4 (a) open-world negation | Whether open-world negation tracking is built at all. (A4's clause-1 half is D1775, closed as DESIGN 2026-09-06 — a written negation type is refused in every annotation position — and needs no ruling.) |
...
| range iteration — a half-open sibling for `to` | Whether the counted loop gains a half-open form and how it is spelled — **`until`** (recommended: keyword, fits `in`/`to`/`step`, Kotlin precedent) vs **`..<`** (symbolic, Swift/Nim). `to` stays inclusive either way; the design doc recommends adding `until` as the PRIMARY taught index-iteration idiom (`for i in 0 until a.length`) plus a lint on `<lo> to <expr>.length`, closing the array-iteration off-by-one (`for i in 0 to a.length` traps) without flipping `to`'s inclusive English reading. Additive — no existing loop changes meaning. Survey, the Dijkstra/Kotlin argument, three options and the recommendation: `docs/internals/range-semantics-design.md`. Ratifying it also files a build item (parse `until`, emit `<` for `<=`, the empty-range-is-legal const rule, the lint). |
...
| `plumb:` `std:fs` writes only whole files, from a `u8[]` (PL-020, PL-009) — **RULED (owner, 2026-09-23), awaiting BUILD** | One owner question remains before the build: `appendTextFile`, add it or refuse it in the header. The std review asks for one or the other. Ruled: `writeFile` widens to `data: u8[] \| Buf`, and `writeFileRange(path, offset, data)` and `appendFile(path, data)` take the same union. There are no `…From` variants. A partial `Buf` write goes through a new copy-free, bounds-checked `window` helper in `std:buffer` (the name the std review chose). Writes are contiguous for now: an offset past the end is `EINVAL`, and the rule may relax later. There is no truncate primitive (`writeFile(path, [])` starts fresh) and no file handle until scope-exit cleanup exists (row below). Surface, build items 1–7 and self-review: `docs/internals/fs-streaming-design.md` §5–§7. |
| scope-exit cleanup (`defer` / `using` / `Drop`-like) | Which construct, if any, lets a block run code on every exit. It gates a `std:fs` file handle (fs-streaming Q1, RULED 2026-09-23: no handle until this exists) and any future std resource. VL has no destructor, `defer` or `using`, and WasmGC has no finalizer. Options and the questions a design owes: `open-rulings.md` §D `scope-exit-cleanup`. |
| `dogfood:` `std:fs` cannot make, empty or remove a directory | Whether `makeTempDir(prefix)` lands first, whether `makeDir`/`removeDir`/`removeFile` do, or both. `listDir` reads a directory and nothing creates one — measured, a write into a missing directory is `ENOENT` — and this BLOCKS the second scheduled port: `scripts/check-filed-witnesses.py` writes each witness to a fresh temp directory, and **eight rows** (D1120, D1191, D1519, D1571, D1585, D1598, D1651, D1654) carry a multi-file witness whose relative imports force the sibling to be named by the WITNESS, so a shared flat directory collides between the six concurrent agents CLAUDE.md allows. Options, peers and a recommendation for `makeTempDir` first: `open-rulings.md` §D `fs-directory-mutation`. |
| `dogfood:` VL has no pattern matching over text | Whether character scanning stays the answer (with `std:str` growing the helpers ports reach for), whether a restricted `std:regex` is built, or whether a checked pattern literal enters the language. NOT a blocker, and that is measured: a VL row parser with no regex reads an inventory row and agrees with `check-filed-witnesses.py` on the id, the status line and the repro's line count — the heading scanner is fifteen lines where `re` is one. This is about the cost the next twenty ports pay. Options, peers and a recommendation for character scanning: `open-rulings.md` §D `text-pattern-matching`. |
| `dogfood:` `runProgram` cannot bound how long a child runs | Whether a `timeoutMs` argument joins it, a `runProgramFor` sits beside it, or callers spawn `timeout` themselves. `runProgram` waits with no bound and there is no `kill`; the Python it would replace carries `timeout=120` on every one of its two-or-three spawns per row across 829 rows, and the programs it runs are by construction ones that provoke the compiler. A robustness regression rather than a blocker. Options, peers and a recommendation for a third argument: `open-rulings.md` §D `process-run-timeout`. |
...
- ✅ **Explicit numeric conversion syntax — SHIPPED; this entry was STALE.** The lossless-only
  ...
  **exists**: `as` covers every edge this item named — `i32 as f32`, `i64 as f64`, and the
  ...
  - **RULED 2026-09-02 (owner): out of range is neither a trap, a saturate nor a wrap — it is
    ...
    the build item.
  - **RULED 2026-09-07: saturation does NOT become a fifth `as` spelling.** A caller that wants
    clamp-to-range gets a `std:math` helper, not a new cast operator — the operator surface
    stays at four (`as`/`as?`/`as!`/`as%`). `docs/internals/numeric-determinism-rulings.md` §2
    (DECISIONS.md §"Float→int out-of-range casts keep the trio; saturation is a std helper, not
    a fifth operator"); the helper itself is a `std:math` build item (row 34, below).
...
- 🟡 **B7. Strings — THE REPRESENTATION MIGRATION IS SHIPPED.** Steps 0–2c are in:
  ...
  **WHERE THE +43% ACTUALLY LIVES, measured** (same profile, self-times, master):
  ...
  3. **`s.backwards()` is BUILT** (`std:str`, ROADMAP row 5) — it reverses the
     code-point sequence UTF-8-aware, so a multi-byte code point stays whole.
