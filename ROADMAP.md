...
| 3 | **[D1774](internals/inventory/D1774.md) — `is <union alias>` has no interned arm representation** — the residue of D1744, closed 2026-09-06, whose checker refusal was masking it | `type U = i32 \| string` + `function f(d: U \| boolean) { if d is U { print(1) } }` → `vl check` rc 0, then `emitProgram: is names a declared union member with no interned arm representation`. The neighbours run: `is C` over `C \| boolean` for a struct alias `C`, and `d as? U` | `wasmEmit.vl:2867` `variantIndexOf`, plus the sibling floor at :2871 the whole-union self-test reaches | days (rep) |
