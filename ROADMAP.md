...
| `plumb:` `std:fs` writes only whole files, from a `u8[]` (PL-020, PL-009) | Whether the write side gains positional and `Buf`-sourced writes (`writeFileRange`, `writeFileFrom`, `writeFileRangeFrom` — recommended), an `appendFile`, or a file handle with an explicit close. Today a 256 MB output must be materialised as one `u8[]` copied out of the `Buf` it was built in, and a file larger than the heap cannot be written. Options, peers, a measured reopen cost and six owner questions: `docs/internals/fs-streaming-design.md`. |
