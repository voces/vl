...
| 29 | ✅ **F-day-one — the five grammar axes are in** | `day-one-sampler.md`: `match` as a `narrowing` spelling (D1885/D1886), multi-param generics as a `pinning` face (D1887/D1888), recursive types as four records with a non-recursive control (D1889), mixed-width arithmetic as eleven READS at zero (D1890), operator overloading as the `operator_vs_call` axis with a UFCS control (D1891-D1894) | `scripts/day-one/grammar.py` | done |
