...
| `crFreshNames` (`crFreshRoot`, D2584) | the root's node INDEX; reset by `crReset` | none: the fresh `const` names are read off the syntax tree, which the checker does not rewrite | no-refined-input |
