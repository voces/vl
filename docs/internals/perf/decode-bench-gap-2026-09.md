...
| 2 | **L4: emit-time inlining of small leaf functions plus scalar replacement of non-escaping structs.** `D` is the textbook case: built in `decode`, passed only to helpers that are candidates for inlining, never stored | **−10 to −14%**, and fewer collections as a side effect | M–L: needs an escape rule and the inliner to run first. A struct passed to a non-inlined call escapes | medium: an escape check has to be exact about closures, `self` arguments and returns. Needs a byte-identical-output check on the corpus plus the rep-fuzz gate | L4 (**landed for `-O`/`-O3` only**: a host step inlines the helpers a per-call struct is passed to, and `-O` gets −14% on wasmtime, −6% on V8 against master's `-O`. The default build needs emit-time inlining plus scalar replacement, measured ceiling −12 to −15% on wasmtime: DECISIONS.md, "`-O` inlines the helpers a per-call struct is passed to") |
| 3 | **L9: an inline length test in front of `__str_eq__` for `==` against a literal** | **−3%** | S | low: a pure pre-check, the call stays the fallback | L9 (landed for a literal operand only: −2.3% to −5.4% on wasmtime, V8 unchanged) |
