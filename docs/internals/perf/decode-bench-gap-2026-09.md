...
| 3 | **L9: an inline length test (and `ref.eq`) in front of `__str_eq__` for `==` against a literal** | **−3%** | S | low: a pure pre-check, the call stays the fallback | L9 (landed for a literal operand only: −2.3% to −5.4% on wasmtime, V8 unchanged) |
