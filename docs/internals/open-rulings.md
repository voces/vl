...
**NOT blocked by this, and it used to be filed as if it were.** The NUMERIC literal-union family (`type K = 1.5 | 2.5`) is outside both A16 rulings entirely: its members are `i32`/`i64`/`f64` VALUES, it reps as its base scalar (`numLitUnionBaseTy`, stated verbatim in `tyKindOf`'s own literal arm), and there is no interned atom ID for a tag scheme or a `ref.i31` payload to encode. Two soundness fixtures had attributed themselves here — `xfail-false-reject-numeric-litunion-array-in-signature` said "it closes with A16's numeric-literal-union representation, not before" — and both were missing ARMS in the string-only ladders (`tyIsLitUnion` demands `litKind == "str"`), closed without touching either ruling. The one numeric cell that IS an A16 question is `x is K` over `K | f64`, where both arms rep as the same f64 and the box collapses; it is a loud REJECT, not a miscompile, so it does not raise the urgency of either fork.
...
**UPDATE 2026-08 — two premises of this filing are now FALSE, measured.** (1) **The perf trap is NOT closed.** B7b's string-accumulation fusion shipped in `compiler/toWasm.ts` (PR #168) and was deleted with the TS core (PR #466); it was never ported, and `grep -rn "accumulat" compiler/*.vl` finds no recognizer. `s = s + "abcdefgh"` in a loop measures 313 / 1502 / 7614 ms at 20k / 40k / 80k appends — quintupling per doubling, i.e. quadratic. The `tests/cases/strings/accum-*.vl` fixtures still pass because they assert only the RESULT, which per-append concat also produces; they are blind to the cost class they were written to pin. So the "builder type for the accumulations fusion cannot prove" sub-question is now "a builder type, or re-port the fusion, for accumulations generally" — and it is the one part of this entry that is urgent rather than ergonomic. (2) **`trim`/`replace` now have a home.** `std:str` ships them (with `split`/`join`/`repeat` moved into it and re-exported from `std:fmt`, so there is one implementation), which settles 5 of 5 method placements at the `std` tier; what remains of that sub-question is only OQ-3's core-vs-`std` line. The remaining live halves of this entry are the builder type, interpolation, and `format(...)`.
...
**A witness was raised against this entry on 2026-09-07 and then WITHDRAWN.**
[D1866](inventory/D1866.md)'s nine cells were briefly filed as this ruling witnessed at the
string-keyed spelling; re-measuring the same day showed they are a NARROWING defect — a
map-valued union member re-read inside its own narrowing, which runs as soon as the read is
bound once — and not this position at all. A map IS a usable union member from a binding, from
a list element, and from a map value bound once. This entry's own justification and its
i32-keyed refusal are unchanged, and it remains open on (a)/(b)/(c) as written above.
...
### dead-null-test — is a null test on a provably non-null value an ERROR or a LINT? — RULED (a) 2026-09-26

**Ruling (owner, 2026-09-26):** (a), following TypeScript's split. A null test (`==`/`!=`
`null`) on a value proved non-null is a WARNING and folds to its constant; an impossible
`match` arm, `is` member or literal comparison stays an ERROR (D2198 stands). BUILT in #3184,
which also closes [D1848](inventory/D1848.md); DECISIONS.md, the D2390 section.
...
| `std-OD1-release-packaging` | **SUPERSEDED** | Option (a) — a `std/` copied beside the binary — shipped and then FAILED in the field: it is how a released `vl` paired a master seed with a checkout 37 commits behind, silently (D1573). The ruling now is that std is BAKED IN and the sibling directory is ignored: DECISIONS.md §"std ships inside the binary; a pin is one file; development overrides are explicit and announced", `docs/internals/cli-design.md` §"Where a `vl` binary finds std and its seed" |
...
| `std-OD1-release-packaging` | **SUPERSEDED** | Option (a) — a `std/` copied beside the binary — shipped and then FAILED in the field: it is how a released `vl` paired a master seed with a checkout 37 commits behind, silently (D1573). std is BAKED IN now and the sibling directory is ignored (with a note): DECISIONS.md §"std ships inside the binary; a pin is one file; development overrides are explicit and announced" |
