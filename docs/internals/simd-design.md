...
- **New intrinsics** declared in `typecheck.vl` (the `declare(...)` block that holds `__load_f32__`
  ...
  `__cmp_lt_f32x4__`, `__bitselect_v128__`, `__shuffle_*__`, … one per instruction the surface needs (~60
...
| shape changes | `narrow_i16x8_s/u`, `narrow_i32x4_s/u`; `extend_low/high_<src>_s/u` for i8x16, i16x8, i32x4; `trunc_sat_f32x4_s/u`, `convert_i32x4_s/u`; `promote_low_f32x4`, `demote_f64x2_zero`, `convert_low_i32x4_s/u` (to f64x2) |
| float min/max | `pmin` `pmax` on f32x4 and f64x2: `b < a ? b : a` and `a < b ? b : a`, each lane one operand's bits unchanged, so x86 `minps d, s` is `__pmin_f32x4__(s, d)` bit for bit, NaN and ±0 included |
...
from an independent model of the spec's lane semantics and run on V8 and wasmtime; the float
rows are `tests/cases/simd/float-pminmax.vl` and `float-convert.vl`, from
`scripts/gen-conv-cases.py`, whose `--verify` checks each opcode against the spec table and
binaryen's decoder. Not in tier 1: `load*_lane`/`store*_lane`, the rest of the `f64x2` shape
(only its pmin/pmax and the three conversions above exist, for plumb's `cvtps2pd`, `cvtpd2ps`
and `cvtdq2pd`), and relaxed SIMD (O6), which stays out because its results differ by host.

**The scalar twins are not dunders.** The one-instruction scalar integer ops have no
`__clz_i32__` spelling, because they already exist as bare builtins
(`docs/internals/numeric-intrinsics.md`): `clz`, `ctz`, `popcnt` (one argument), `rotl`, `rotr`
(two), at i32 or i64 by the operand's width, each lowering to the one wasm instruction. At i64
all five answer an `i64`, as `i64.clz` does. They are bare because they are safe and total; a
`__…__` name is for raw, address-level or vector machinery. A program's own function of the
same name shadows the builtin. Graded against an independent model on V8 and wasmtime in
`tests/cases/numerics/int-opcodes-model.vl`.
...
which the host passes on every build and which changes no bytes of a module that uses none.
Graded in `tests/cases/numerics/trunc-sat.vl` (generated with the float rows above).

**Atomics.** The wasm threads proposal's `0xFE` instructions are a second table of the same
kind (`atomicTableBuild`, beside `simdTableBuild`), approved by the owner on 2026-09-24 as raw
opt-in intrinsics for plumb's parallel guest threads. They act on linear memory only, never on
a GC object, and there are no std exports. Names follow the rule above with `atomic` kept in
the op: `i32.atomic.load` is `__atomic_load_i32__`, `i64.atomic.load16_u` is
`__atomic_load16_u_i64__`, `i32.atomic.store8` is `__atomic_store8_i32__`,
`i32.atomic.rmw8.add_u` is `__atomic_rmw8_add_u_i32__`, `i64.atomic.rmw.cmpxchg` is
`__atomic_rmw_cmpxchg_i64__`. The `memory.` ones drop the prefix: `__atomic_wait32__(addr,
expected: i32, timeout: i64): i32`, `__atomic_wait64__(addr, expected: i64, timeout: i64):
i32`, `__atomic_notify__(addr, count): i32`, and `__atomic_fence__()`.

| family | intrinsics (each at `i32`, `i64`, and the narrow widths 8/16 of i32 and 8/16/32 of i64) |
| --- | --- |
| load | `__atomic_load{,8_u,16_u,32_u}_<shape>__(addr)` |
| store | `__atomic_store{,8,16,32}_<shape>__(addr, value)` |
| rmw | `__atomic_rmw{,8,16,32}_<op>{,_u}_<shape>__(addr, operand)` for `add` `sub` `and` `or` `xor` `xchg` |
| cmpxchg | `__atomic_rmw{,8,16,32}_cmpxchg{,_u}_<shape>__(addr, expected, replacement)` |
...
`--enable-threads` on every build, since it changes no bytes of a module that has no atomics.
wasmtime validates atomics on an ordinary memory with no config change. Until a build can
declare a `shared` memory, which is a separate lane, `wait` traps and `notify` answers 0, as
the proposal specifies for a memory that is not shared. The graded evidence is
`tests/cases/atomics/`, generated by `scripts/gen-atomic-cases.py` from an independent model
...
shifts) at counts at or above the width; `__swizzle_i8x16__` zeroes for an index of 16 or more,
where `pshufb` reads only bit 7 and the low 4 bits, so mask the index with `0x8F` first; and
`__q15mulr_sat_s_i16x8__` saturates `0x8000 * 0x8000` to `0x7FFF`, where `pmulhrsw` wraps to `0x8000`.
