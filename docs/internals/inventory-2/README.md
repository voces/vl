...
template is [`../inventory/TEMPLATE.md`](../inventory/TEMPLATE.md). All 17 rows carry a
`Repro:` label over their program: the unlabelled-block fallback the grader once had is
retired, because prose refuses like a type error and an indented paragraph graded as a
witness for weeks.

These rows are in the `filed witnesses` gate row alongside inventory #1, so they are re-run
on every merge rather than only when someone remembers to:
