...
"an empty block does nothing; remove it" and is listed in `stmtBraceNodes` so the checker
types it as the error type without a second message. A `{}` in expression position is the
checker's: it completes an expected record whose fields all admit `null`. The empty BODY of
a construct (`if c {}`, `function f() {}`, `() => {}`, a match arm's `=> {}`) never reaches
either: those callers take `parseBlock` for empty braces.
