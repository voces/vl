...
| **union returns are ALWAYS explicit; scalar returns are usually elided** | **QUOTE THE COMMAND, NOT THE NUMBER** — this row's counts have been re-derived four times and three of the four were stale within a month. `grep -h '^export' std/*.vl` gives **128** statements (2026-09-03), minus `std/seed.vl`'s `stdSmoke` (a smoke hook, not a surface); an exported-NAMES count is a different number again, because of the four `"[]"`/`"[]="` operator forms and the re-exports in `std:args` and `std:fmt`. The half that IS the convention is exceptionless and worth checking: **25 of 25** union-returning exports annotate (2026-09-23, after `appendTextFile`) — `decodeUtf8`, `decodeUtf8At`, `readFile`, `readTextFile`, `readFileRange`, `fileSize`, `readFileInto`, `readFileRangeInto`, `writeFile`, `writeFileRange`, `appendFile`, `writeTextFile`, `appendTextFile` (the five writes via the `IoResult` alias), `listDir`, `pathKind`, `pathExists`, `programArgs`, `getEnv`, `runProgram`, `parseF64`, `parseI64`, `parseI32`, `decodeBase64`, `parseJson`, `toJson` — while `utf8Length`/`join`/`repeat`/`padLeft`/`toString`/`decodeUtf8Lossy` elide. The scalar half is taste, not convention. (`toString`'s union is in its PARAMETER, which this row does not govern.) |
