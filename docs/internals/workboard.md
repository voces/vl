...
**This corrects the framing I shipped two commits ago.** I wrote that the emitter half was "NOT
blocked on B5", and that is still true — all three slices landed without it. What is now measured is
the narrower and more useful claim: **B5 owns 67% of what is LEFT.** The canon gate is exactly the
`nodeTyIx`-goes-stale-at-canon condition B5's lockstep write removes, so taking B5 would move leaf
coverage 92.8% → about 97.6%, and nothing else on this list would move it more than a point.

That is the number the B5 ruling should now be weighed against — not the "970 of the remaining
`annotResolve` parses" figure from before these slices, which the slices themselves have overtaken.
