...
| — | **An inferred `Map()` does not lower in TWO composition positions** — surfaced while gridding E2, and the `.set` spelling already exposed both on master | EMITTER shape ladder, not the checker: `mapShapeOfExpr`'s `Call` arm returns the callee's DECLARED return shape before the recorded-type fallback; and an object literal carrying a hole map interns no row | **OPEN, UNFILED, MEASURED HERE: 288 cells, all with an `ann=OK` oracle.** `function mk() { return Map() }` + `const m = mk()` + any write ⇒ `emitProgram: map values must be i32 / boolean` (144); `const s = { m: Map() }` + `s.m[k]=v` ⇒ `emitProgram: ref valtype with no interned shape` (144). Both are LOUD rejects, both run when the map is annotated, and both are independent of the key rep. Not folded into E2: E2 is a checker pin, this is the emitter's shape resolution | M | med — rep-adjacent |
