...
|---|---|---|
| **the narrowing overlay leaking into ASSIGNMENT TARGET types** | loud FALSE reject — `while q != null { … q = null }` gives `cannot assign null to i32` although `q` is declared `i32 \| null` | bundles #1444's refuted premise and its residue (d). **A widening** — retiring the narrowing at the write is mandatory, and the loop back-edge question is its own deliverable |
| **the three COMPILER TRAPS** | `vl check` dies inside `vl-compiler.wasm`, **no diagnostic, no module written** | Shape A: an empty array literal returned from an annotated lambda (3 lines, I reproduced it). Shape B: a string-**VALUED** captured map. Shape C: the sweep's original 4 cells. **How many roots is the question** — an outcome groups unrelated defects |
| **#1444's `findFnDeclIn` is QUADRATIC** | +15 / +61 / +227 ms at 750 / 1,500 / 3,000 guards; ~4x delta per 2x input; per-guard cost doubling 20 → 40.7 → 75.7 µs | I measured it; the slice is to fix it. The correctness question is whether the walk is **scope-sensitive**, since a flat map that wrongly answers "found" fails **open** |
