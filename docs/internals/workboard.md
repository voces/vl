...
| **D9** | **`s?.f is T` over an OPTIONAL-CHAIN receiver answers a constant FALSE** where the field is a NICHE nullable or a ref-element array | `wasmEmit.vl emitIs`'s `OptMember` arm keys on `sFieldTypeAt(si, fi) == 16` (a union-BOX field); a niche `string \| null` / `T[] \| null` / a `string[] \| i32[]` field is not code 16, so the arm is not reached | **MEASURED** as a D6 control, 9 unions × 2 builds: `{f: string \| null}` holding `"hi"` prints **no** for `s?.f is string`; same for `i32[] \| null`, `F \| null`, `F[] \| G[]`, `string[] \| i32[]`. `i32 \| null`, `f64 \| null`, `string \| i32`, `i32[] \| string` (all real boxes) are CORRECT. Fails CLOSED, `vl check` rc 0. **Not function-specific** — surfaced by the D6 grid, owned by the optional-chain arm | OPEN, UNFILED before now | S–M | low — fails closed |
| **D10** | **A bound map read of a niche-nullable value emits INVALID WASM with no `is` in the program** | `{[string]: F \| null}` where only `null` is ever stored: `const r = mp["k"]` → `Invalid input WebAssembly code … expected (ref null $type), found (ref null $type)` — two distinct type indices printing the same name (a duplicate array type minted because the closure type is never interned) | **MEASURED** as a D6 control and **isolated**: deleting the `is` guard entirely still emits invalid wasm, so it is a map REP hole, not an `is` hole. Twins: `i32[] \| null` and `string[] \| i32[]` are EMIT-REJECT (loud), `string \| null` is RUN-WRONG (silent). Storing one real closure alongside the `null` makes it compile and answer correctly | OPEN, UNFILED before now | M | med — the `F \| null` spelling is the INVALID-WASM one |
| **D11** | **A degenerate ONE-member union (two aliases with the same structure) breaks `is`** — `type A = i32; type B = i32; x: A \| B` → `x is A` prints **notA**, silently | the arena collapses the pair (`sameVariantTy`), so there is no union left for the tag compare to read | **MEASURED** as a D6 control. The FUNCTION spelling (`type F = (i32) => i32; type F2 = …` identical) is the LOUD one — *``is` test but no union type declared`* at 6 of 7 receivers — while the scalar spelling is the silent wrong answer. VL is structurally typed, so `is A` here is soundly always TRUE and both answers are wrong | OPEN, UNFILED before now | S | low |
