...
| **`==` between two NULLABLE LISTS** | **check-clean INVALID WASM**, and **rep-FLAT** — all six element types (`i32[]`, `string[]`, `f64[]`, `i64[]`, `boolean[]`, `i32[][]`) fail identically | two working controls: the **non-nullable** list `==` prints `1`, and `== null` on the same type prints `0`. So the rep has a working null test AND a working non-null `==`, and only the combination has no lowering. The semantics of `null == null` is a real choice and must be justified against the non-nullable oracle (reference vs structural equality) |
| **D1d capture-frame typing** | in flight | re-derivation moved the filing BOTH ways: the litunion-atom half is closed, and an `i32\|null` captured under `!= null` (**check-clean invalid wasm**) is a shape the filing never named |
| **discovery sweep** → `docs/internals/silent-class-inventory.md` | in flight | fixes nothing; produces a ranked inventory of live defects, each with a minimal repro AND a working control, plus a not-a-defect section and a shared-root analysis. **Brief from the inventory, not from the historical queue** |
