...
| **D1e** | **An un-annotated (monomorphized) param is assignable to a `K` slot with no narrowing** — `function p(u) { const r: K = u }` | measured beside D1a | **A CHECKER hole first, invalid wasm second.** `p("zz")` — a NON-member — is `vl check` rc 0 with no error, i.e. the hole launders an arbitrary string into a literal-union slot; the annotated twin `p(u: string)` is correctly rejected (`cannot assign string to 'r' of type "x" \| "y"`). Today it cannot be observed as a wrong answer only because the emitter then produces invalid wasm (`expected i32, found (ref $type)`). Lowering it with `emitStrToAtom` would be WORSE than the reject — the tower's last member is the unconditional default, so `"zz"` would silently become a member. 6 of 900 D1a grid cells | OPEN — needs the checker ruling before any lowering | S (reject) / M (narrowing requirement) | med — a lowering here is a silent-corruption risk |
