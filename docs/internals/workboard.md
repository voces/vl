...
| **D1i** | **`u == kk` against a litunion alias is invalid wasm, and holes are not involved** — `const kk: K = "a"` + `function p(u: string) { print(u == kk) }` | measured beside D1e | The `u: string` ANNOTATED control fails identically to the un-annotated spelling (`expected i32, found (ref $type)`), so this is a `string`-vs-atom `==` rep defect on its own, not a parameter-inference one. `binOpDefinedFor` admits the compare (a litunion IS assignable to `string`), and the emitter then hands `__str_eq__` an i32. 16 of 840 D1e grid cells, every one reproducing with the parameter annotated | OPEN — `==` operand rep, one of the two sides must widen | S–M | med — silent for the boolean result, invalid wasm for the module |
| **D1j** | **`u is Z` over a NUMERIC litunion answers the wrong branch for a hole parameter** — `type Z = 0 \| 1` + `function p(u) { if u is Z … }` + `p(1)` | measured beside D1e | Prints the ELSE arm for a value that IS a member; the `u: i32` control prints the then-arm. String litunions are unaffected (both spellings answer correctly), so it is the numeric membership test over a hole-typed receiver. 2 of 840 D1e grid cells, and the only SILENTLY-wrong pair left in that grid | OPEN — numeric membership over a hole | S | med — silent |
