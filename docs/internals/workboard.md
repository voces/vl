...
|---|---|---|
| **1 — destringify** | **B2**, the TRANSP residue: a generic ALIAS loses its name | `type Y = Box<i32>` renders **`{v: i32}`**, not `Box<i32>`. The program runs — this is type *identity*, not codegen. The board says the rung was already BUILT and MEASURED (B2 20→11, B3 120→111, moves nothing on six channels, costs ONE cell) but **both halves of the old filing were REFUTED**, so the agent re-derives rather than inherits |
| **2 — webcraft** | **C8's silent half**: array element subtyping | `Cat[]` passed where `Animal[]` is expected is **`vl check`-clean and emits invalid wasm**, and the board notes it is **"not even pinned as xfail"**. The full variance feature stays blocked; the slice is to make the hole **loud** and pin the boundary in both directions so a later implementation has a spec to move against |
| **perf** | the filed **`cFillDoneIx` O(#typedecls²)** and a sweep for siblings | filed by #1448 and explicitly **not profiled**. Briefed with #1449's cautionary tale attached: that slice's magnitude was right and its *mechanism* wrong, so the attribution must be proved by **probe-sabotage**, not assumed |
