...
| item | state |
|---|---|
| **the narrowing overlay leaking into ASSIGNMENT TARGET types** | **#1451 open.** One root; sabotage reproduces master's 516-cell grid cell-for-cell. correct **0 → 108**, newly rejected **0**, every silent column 0 both ways. Loop bodies got a **re-execution barrier** (not the back-edge pre-pass I suggested), justified at runtime: barrier off with the loop running twice, `i32\|null` **traps** and `boolean\|null` prints **`true\|true\|end`**. Six sabotage configs **all exact**; the two legs partition 8 + 100 = 108 with nothing left over. **It closed an unsoundness #1446 created while it ran**, with a function-body epoch counter. And it turns four pre-existing **check-clean** defects loud — including `const q = …; if q != null { q = 5 }`, which on master is check-clean and **prints 5** while the same write outside a guard is correctly rejected: **`const` protection silently vanished inside every guard** |
| **second discovery sweep** → `silent-class-inventory-2.md` | in flight; covers the first sweep's declared gaps (sets, newtypes, nested arrays, generics, `flat` records, multi-module, `-O` output, struct-field ASSIGNMENT, capture chains >2 levels, and **evaluation counts inside `is`/`match`**) |
