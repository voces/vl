...
| **B2e** | **the display renderer emits three spellings VL cannot parse** — the arrow `->` (VL spells `=>`), the nullable suffix `T?` (VL spells `T \| null`) and NO grouping at all (`(A\|B)[]` renders `A \| B[]`). Measured by substituting each `redundant type annotation` render back as its own binding's annotation: **1,298 of 1,877 round-trip** corpus-wide, and the 579 failures are 280 arrow · 113 nullable · 101 grouping | `typecheck.vl` `tyToStrGo`'s `TyFunc`/`TyNullable`/`TyArray`/`TyUnion` arms; `arrElemRender` is the grouping home the two EMIT renderers already share and this one does not | each leg is independently measurable and each moves its own set of corpus `@hint` pins | OPEN | M | med |
