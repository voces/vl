...
| **D7** | **`%` with a FLOAT operand emits invalid wasm** — `vl check` clean, then `type mismatch: expected i32, found f64` at instantiate | `emit_base.vl:552 binOpcodeF64` / `:591 binOpcodeF32` return **−1** for `%` (wasm has no `f64.rem`); the consumer `wasmEmit.vl:15514` guards `if opcF >= 0` and **falls THROUGH** the f64 and i64 slices to the i32 tail, which emits `i32.rem_s` (0x6f) over f64 operands | **MEASURED, re-verified here.** Fails with BOTH operands annotated, so it is an EMITTER gap, not inference: `f64 % f64`, `f64 % i32`, `i32 % f64`, `f32 % f32` (annotated `const`s) and the bare-literal `5.5 % 2.0` all check clean and emit invalid wasm. In the 44-cell nested-capture grid below it is the **only** annotated-spelling failure (1 of 44). A sibling grid measured **24 of 1,320** (2 pairs × 12 spellings) — that is a FLOOR: it omits `i32 % f64` and the f32 family | OPEN | S (loud reject) / M (lower as `a − trunc(a/b)*b`) | low — the fall-through is the defect; a `-1` opcode must never reach the i32 tail | 
| **D8** | **Un-annotated nested function that CAPTURES** — the capture path lowers it through the function-VALUE ABI, where an un-annotated param defaults to i32 | `wasmEmit.vl:1404` (the reject the i32 arm hits); the value ABI is ROADMAP B15 | **MEASURED HERE**, 44-cell grid (4 operand types × 11 operators), two spellings each — `function o(n) { function k(x) x OP n; k(LIT) }`: **f64 → 11 of 11 INVALID-WASM** (`vl check` clean, `expected i32, found f64`); **i32 → 11 of 11 loud REJECT-EMIT**; string `+`/orderings and boolean `==`/`!=` PASS. The ANNOTATED twin is clean in **all 22**, so **22 of 44** bare cells are broken — **21** once `f64 %` is charged to D7, which reconciles the sibling's 21. Pre-existing. `string + string` under this spelling was named in the filing but does **not** reproduce: it PASSES | OPEN | M | med — the f64 half is SILENT; the i32 half is already loud |
