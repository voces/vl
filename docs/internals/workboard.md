...
| item | state |
|---|---|
| **Root A — `monoArgTyName`'s missing arm-set** | `emit_mono.vl:315` has no arm for `string`, maps or `new string` in its annotated-local and param branches, **and the loud floor is in the same function** which these spellings route around: one arm-set is **44 silent cells vs 44 loud**. Re-derived on the tip: `gtake<T>(x)` printing `x[0][1]` over `boolean[][]` prints **`0`** where the direct read prints **`false`** (SILENTLY WRONG VALUE), and `const y: string = "aa"; gid(y)` is **check-clean invalid wasm** while dropping the annotation prints `aa` |
