...
| **C8** | **Readonly fields / A9 variance** | ROADMAP `:780`, `:782` | **the SILENT half is closed and the boundary is pinned in both directions.** The filed axis was wrong twice over: not the array element (a BARE binding breaks identically — no container involved) and not width-vs-depth, but the source's shape PROVENANCE. A PINNED value (parameter / callee return / field read / annotated binding) cannot reach any different shape; an UN-ANNOTATED binding of an object literal is re-interned at the destination row, so reorder, a widened field type, and a NESTED dropped field all WORK there — same type pair, opposite verdicts. Only a TOP-LEVEL dropped field is unconditional, and that is what `objShapeAdapterless` now rejects loudly from `assignableExpr` on the `unsupported-lowering` channel. 337-cell grid: invalid wasm **124 → 79**, program trap **17 → 5** (a function-value pair VALIDATES and traps at the indirect call), attributable hole **109 → 55**, 0 regressions; corpus 1,962 files **0 verdict changes, 1,609/1,609 modules byte-identical**. **RESIDUE, all provenance-conditional and needing the emitter's adapter (i.e. this item):** 43 depth cells + 10 field-nested width cells. **A 24-cell pre-existing defect the grid found:** `Wrap<Sub>` → `Wrap<Sup>` with two aliases of ONE shape is check-clean invalid wasm on master — the generic-argument row's own identical CONTROL is broken, so it cannot speak to variance. **A worse class found beside it:** an object literal's EXCESS field is dropped together with its initialiser's EVALUATION (`{ legs: 4, purr: sideEffect() }` checks clean, builds a valid module, prints the right number, never calls `sideEffect`) — 6 of 7 positions, check-clean **wrong evaluation count**, unfixed | **BLOCKED** on N5 for the remaining half | L | high |
