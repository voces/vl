...
| **F6b** | **the `.cwasm` cache key omits the ENGINE CONFIG** — `check`/`fmt`/`test` (`Collector::Auto`) and `build`/`run` (`Collector::Null`) compile the seed differently and publish to the same `<seed>.cwasm`, keyed by mtime freshness alone. Alternating them recompiles the ~1.2 MB seed every invocation | `perf-program.md §18.2`, `main.rs:462` | **~2 s and ~600 MB per invocation instead of ~6 ms / 20 MB** — a FIVE-LINE file reads 610 MB. Both the on-disk sidecar and the embedded `seed-<VL_SEED_KEY>.cwasm` key on module bytes only | OPEN, sized | S | low |
