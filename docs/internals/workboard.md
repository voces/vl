...
| **H3** | **F-tiers residue** | OPEN, **row RE-SCOPED in `ROADMAP.md`**. `SELFHOST_DENO_RUN` is gone; the residue is **four** tests executing emitted wasm under V8 via `tests/support/runWasm.ts` — `cases_wasm_*_test.ts` (the sole behavioral corpus oracle; must be re-hosted, not deleted) + `vl_exported_memory` / `vl_global_promotion` / `vl_reexport_abi` |
...
- Adding a corpus case ⇒ run `deno test -A --parallel tests/cases_wasm_*_test.ts` and
  register any divergence in `tests/support/casesWasmOracle.ts`'s
  `EXPECTED_DIVERGENCES` **in the same PR**, or master goes red on merge.
