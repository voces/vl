...
close for 11 cells and a CLAUSE-2 close for 52 · witness respelled with a `readonly`
destination on 2026-09-23, since only a readonly list is covariant ([D2161](D2161.md)); the
mutable spelling is refused by design, and the copy this row built now serves `readonly`
destinations only**
