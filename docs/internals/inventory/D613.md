...
**graded witness moved to an integer base after the D2221 ruling (a float literal is not a type); the original float-base witness is kept below as history · now RUNS · was `check-clean invalid wasm` · found 2026-08-30 with D611 · closed as `runs`, not as a third refusal: D411's and D501's destination is AMBIGUOUS and this one is not · 17 corpus classes / 310 census cells `check-clean invalid wasm` -> `runs`, 0 `runs` lost, 0 into any silent class, every corpus module that built before builds BYTE-IDENTICAL**

Repro (runs, printing nothing):
...
Original witness, over `type F = 1.5 | 2.5` (refused at `type F` since D2221):
