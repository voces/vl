...
Repro (runs, prints `2` — the `+` over an `i64`-backed literal union):

    type F = 5000000000 | 6000000000

    function cell(): i32 {
      const a: F[] = [5000000000]
      const b: F[] = [6000000000]
      const c = a + b
      return c.length
    }
...
Original witness, over the `f64`-backed union (refused at `type F` since D2221):
