...
**closed 2026-09-06 by #2802 — the Repro prints `hi` then `d` · was a loud emit reject, clause 2 · ZERO corpus cells · found 2026-09-03 as the residue of [D1250](#d1250)**

Repro (prints `hi` then `d`):
...
* **`emitCoalesceNulRefCtx` ALREADY EVALUATES ONCE** (D7 replaced the re-read with
  ...
  measure, not a fix to assume. **Measured, and it was: #2802 removed the gate.** An
  if-expression operand sets `lhsReEval` false exactly as a call does, so one deletion serves
  both, and the ablation's `else { null }` row moves with it.
...
| an explicit `else { null }` added | refused before, runs now | no |
