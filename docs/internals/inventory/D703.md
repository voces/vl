...
* **SHADOWING IS PINNED IN BOTH DIRECTIONS.** A PARAM shadow that must run
  (`generics/pin-global-binding-both-scopes.vl` leg 5) and a LOCAL shadow
  (`generics/pin-global-shadowed-by-local.vl`), which was a loud reject when this row landed
  and RUNS since D705 closed 2026-09-03 — the reject was D705's cascade, not this row's, and
  what the pin owns is unchanged. A four-cell shadow grid is byte-identical to master.
