...
**closed 2026-09-03 · was loud check reject, clause 2 · the DEFERRED remainder (`HD_GNOT`)
carried the `i32 | null` and `i32 | string` spellings to `runs`, and this row's own LITERAL
witness followed the same day when [D1412](#d1412) banked the CALL'S PIN for the emitter's
literal complement · position matrix 35 of 40 with 1 silent → **52 of 52, 0 silent** ·
fixture `inference/hole-param-is-complement.vl`**

Repro (prints `10`; before the fix `vl check` rc 1, while annotating the parameter
`a: i32 | "err"` printed `10`):
...
* **THE HALF THAT CLOSED, AND WHY THE OTHER HALF IS NOT A DESIGN RULE.** `subtractTy` needs a
  ...
  for, and neither is a language decision: a LITERAL-bearing `T` WAS [D1412](#d1412) (the
  emitter's complement could not see a member the registry softened away, and admitting it
  alone was check-clean invalid wasm at two matrix cells and this row's own witness — the
  gate is gone now that `pinnedHoleTyOf` serves it), and a SHAPE `T` would re-base the hole's own field
...
* **BOTH FACES.** Annotated: RUNS, prints `10`. Un-annotated: WAS a `loud check reject`,
  now RUNS and prints `10` too. And the
...
* Probe: `scripts/capability-probes/is-complement-hole-param.vl` (RUNS).
