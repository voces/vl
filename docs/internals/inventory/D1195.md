...
**closed 2026-09-06 by D1723's argument arm — the witness prints `3` · was check-clean
invalid wasm, `type mismatch: expected (ref $type), found (ref $type)` at `vl check` rc 0,
clause 1, at BOTH same-rep spellings · found 2026-09-02 in [D1050](#d1050)'s position matrix,
where the mixed spelling stopped being a loud refusal and started reaching this instead — the
refusal was a loud floor standing over an older defect, not a statement about the program**
...
Repro:
    ...
    // was: vl check rc 0, then Invalid input WebAssembly code —
    //   type mismatch: expected (ref $type), found (ref $type). Now prints 3.
...
D1723's argument arm. The two heap types are still two — this row's own reading of the type
section stands — and the delivery no longer needs them to be one: the narrowed arm's box is
unboxed to the PARAMETER's own row before the call, so the arm's heap type never has to match
the annotation's. `armDestHeapOf(kind, slot)` over the parameter's rep and table slot is what
supplies that row, and an inline-shape annotation has one exactly as a named struct does.
