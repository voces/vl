...
**AND THE PRICE, FILED AND SINCE CLOSED AS [D1342](#d1342).** The bare-`as` cell runs on a NON-null value, which
...
LEGALITY rather than about a rep, which is worth its own row. D1342 closed that by asking the
propagation obligation at the call that pins the hole, so the VOID-enclosing-function cells of
this grid are the annotated twin's check reject and the DECLARED-return ones run.

Repro (prints `5`; it was `vl check` rc 0 over an invalid module. The bare `as` needs a
DECLARED return to propagate into — the VOID spelling this row filed is now the annotated
twin's loud check reject, which is [D1342](#d1342)'s close, not a regression of this one):

    const u: i32 | null = 5

    function take(a): i32 | null {
      return a as i32
    }
